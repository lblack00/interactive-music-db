    ...
    <transition name="fade">
      <div class="login-prompt-overlay" v-if="showLoginPrompt">
        <div class="login-prompt-modal">
          ...
          <div class="buttons">
            <button class="cancel-btn" @click="showLoginPrompt = false">Cancel</button>
...
export default {
  ...
  methods: {
    ...
    async rateItem(rating) {
      try {
        const sessionResponse = await axios.get('http://localhost:5001/check-session', {
        ...
        const response = await axios.post('http://localhost:5001/rate', {
    ...
    async fetchRatings() {
      try {
        ...
        if (sessionResponse.data.logged_in) {
          const userResponse = await axios.get(`http://localhost:5001/user-rating`, {
            params: {
              item_type: this.itemType,
              item_id: this.itemId
            },
