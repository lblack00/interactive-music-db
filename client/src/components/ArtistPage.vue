...
<template>
  ...
  <v-container v-if="data">
    <v-row>
      ...
      <v-col cols="12" md="9">
        ...
        <h1 id="artist-name" class="text-h4 font-weight-bold">{{ data.artist?.[0]?.name }}</h1>
        <RatingSystem 
          itemType="artist"
    ...
    <div role="region" aria-labelledby="discography-heading" v-if="data.discography && data.discography.length">
      <h2 id="discography-heading" class="text-h5 mt-6">Discography</h2>

      <v-row class="align-center mb-4">
        <v-col cols="12" md="4">
          <div role="search" aria-label="Sort discography">
              <label for="sort-select" class="sr-only">Sort discography by</label>
          <v-select
             id="sort-select"
            v-model="sortBy"
      ...
      <v-row aria-label="Albums List">
        ...
        <article aria-labelledby="album-title-{{index}}">
          <v-card class="pa-2" outlined>
            <v-img src="/images/UnknownSong.png" height="120" contain :alt="`Album cover`"/>
            <v-card-title>
              <router-link :key="i" :to="`/master/${release.master_id}`":aria-label="`View details for album`"></router-link>
            </v-card-title>
          </v-card>
          </article>
        </v-col>
  ...
  <div v-else class="loading" role="status" aria-live="polite">Loading...</div>
  </main>
</template>
