      ...
      <v-col cols="12" md="8">
        <v-card class="pa-4">
          <article role="region" aria-labelledby="thread-title">
            <header>
          <h1 id="thread-title" class="text-h4 font-weight-bold mb-4">{{ thread.title }}</h1>
          ...
          <div class="mb-4">
           ...
           <section role="region" aria-labelledby="replies-heading">
              <h2 id="replies-heading" class="text-h5 font-weight-bold mb-4">Replies</h2>
                ...
                <v-list-item v-for="reply in thread.replies" :key="reply.id">
                  <v-list-item-content>
                    <article aria-labelledby="reply-author-{{reply.id}}">
                      <v-list-item-title id="reply-author-{{reply.id}}" class="font-weight-bold">{{ reply.author.name }}</v-list-item-title>
          ...
          <h2 id="reply-form-heading" class="text-h5 font-weight-bold mb-4">Add a Reply</h2>
          <v-form @submit.prevent="addReply" aria-label="Reply form">
            ...
            <v-textarea
              ...
              aria-required="true"
            ></v-textarea>
            <v-btn type="submit" color="primary" class="mt-3" aria-label="Post your reply">Post Reply</v-btn>
          </div>
          </v-form>
          </section>
        </v-card>
